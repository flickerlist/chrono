...
test("Test - Single Expression", function () {
    ...
    testSingleCase(chrono.zhcn, '星期四', new Date(2016, 9-1, 2), (result) => {
    ...
    testSingleCase(chrono.zhcn, '周四', new Date(2016, 9-1, 2), (result) => {
        expect(result.index).toBe(0);
        expect(result.text).toBe('周四');
    ...
    testSingleCase(chrono.zhcn, '礼拜四 (forward dates only)', new Date(2016, 9-1, 2), {forwardDate: true}, (result) => {
        expect(result.index).toBe(0);
    ...
    testSingleCase(chrono.zhcn, '礼拜日', new Date(2016, 9-1, 2), (result) => {
        expect(result.index).toBe(0);
    ...
    testSingleCase(chrono.zhcn, '周日', new Date(2016, 9-1, 2), (result) => {
        expect(result.index).toBe(0);
        expect(result.text).toBe('周日');
    ...
    testSingleCase(chrono.zhcn, '周末', new Date(2016, 9-1, 2), (result) => {
        expect(result.index).toBe(0);
        expect(result.text).toBe('周末');
    ...
    testSingleCase(chrono.zhcn, '上个星期三', new Date(2016, 9-1, 2), (result) => {
        expect(result.index).toBe(0);
        expect(result.text).toBe('上个星期三');
...
test('Test - forward dates only option', function () {
    ...
    testSingleCase(chrono.zhcn, '星期六-星期一', new Date(2016, 9-1, 2), {forwardDate: true}, (result) => {
        expect(result.index).toBe(0);
